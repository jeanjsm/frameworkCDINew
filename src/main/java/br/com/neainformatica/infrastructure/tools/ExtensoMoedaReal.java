...
/**
 * Metodo para escrever o valor por extenso no formato de Moeda
 * 
 * Para Teste: ExtensoMoedaReal teste = new ExtensoMoedaReal(new
 * BigDecimal(args[0])); log.debug("Numero  : " + (new
 * DecimalFormat().format(Double.valueOf(args[0]))));
 * log.debug("ExtensoMoedaReal : " + teste.toString());
 * 
 * @author Elielcio.Santos
 */
public class ExtensoMoedaReal {
	
	private static final Log log = LogFactory.getLog(ExtensoMoedaReal.class);
	 ...
	 */
	public ExtensoMoedaReal(BigDecimal dec) {
		this();
		setNumber(dec);
	}

	/**
	 * Constructor for the ExtensoMoedaReal object
	 * 
	 * @param dec
	 *            valor para colocar por extenso
	 */
	public ExtensoMoedaReal(double dec) {
		this();
		setNumber(dec);
	}

	/**
	 * Sets the Number attribute of the ExtensoMoedaReal object
	 * 
	 * @param dec
	...
	public void show() {
		...
		while (valores.hasNext()) {
			log.debug(((Integer) valores.next()).intValue());
		}
		log.debug(toString());
	}
	...
	/**
	 * Adds a feature to the Remainder attribute of the ExtensoMoedaReal object
	 * 
	 * @param divisor
